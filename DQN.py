...
from function_tools.Utilities import ReplayMemory, plot_episode
...
class DQN_Agent():
    ...
    def __init__(self, ENV_NAME, BATCH_SIZE, GAMMA, EPS_START, EPS_DECAY, EPS_END, TAU, LR, N_EPISODES, PRINT_PLOT):
        # hyperparameters
    ...
    def select_action(self, state):
        ...
        if sample > eps_threshold: # standard eps-greedy policy
            with torch.no_grad():
                return self.policy_net(state).max(1).indices.view(1, 1) #choose action with the greatest state-action value for exploitation purpose
        else:
    ...
    def optimize_model(self):
        ...
        state_action_values = self.policy_net(state_batch).gather(1, action_batch) # Action taken for each state according to Q(s,a), with a the actions took in previous steps and store in the rollout store
        ...
        # Compute Huber loss --> step of TD(0)
